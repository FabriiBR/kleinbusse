class HomeworksController < ApplicationController
  ...
  def index
    ...
    @assignature_homeworks = Homework.joins(:lesson).where(homeworks: {type_off: "homework"}).where("lessons.assignature_id =?",params[:assignature_id] ).includes(:user_homeworks).where(user_homeworks: { student_id: current_user.id, status: "pendiente" }).or( Homework.joins(:lesson).where(homeworks: {type_off: "homework"}).where("lessons.assignature_id =?",params[:assignature_id] ).includes(:user_homeworks).where(user_homeworks: { student_id: current_user.id, status: "corregir" })).group_by(&:lesson)

    @assignature_flashcards = Homework.joins(:lesson).where(homeworks: {type_off: "flashcard"}).where("lessons.assignature_id =?",params[:assignature_id] ).includes(:user_homeworks).where(user_homeworks: { student_id: current_user.id, status: "pendiente" }).or( Homework.joins(:lesson).where(homeworks: {type_off: "flashcard"}).where("lessons.assignature_id =?",params[:assignature_id] ).includes(:user_homeworks).where(user_homeworks: { student_id: current_user.id, status: "corregir" })).group_by(&:lesson)
